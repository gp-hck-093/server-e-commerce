...
async function insert(id, embedding, metadata) {
  try {
    ...
    let safeEmbedding = Array.isArray(embedding)
      ? embedding.flat(Infinity)
    ...
    if (!safeEmbedding || safeEmbedding.length === 0) {
      logger.error(
        "ALARM! Embedding kosong atau gagal dibuat oleh Hugging Face. Raw Embedding:",
        embedding,
      );
...
async function similaritySearch(queryEmbedding, k = 3) {
  try {
    const safeQueryEmbedding = Array.isArray(queryEmbedding)
      ? queryEmbedding.flat(Infinity)
