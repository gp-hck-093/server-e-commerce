...
async function processUserMessage(message, imageUrl = null, userId = null, chatHistory = []) {
  try {
    ...
    if (
      parsedCommand.action === "ADD_CART" &&
      parsedCommand.productId &&
      userId
    ) {
      try {
    ...
    if (
      parsedCommand.recommendedProductIds &&
      Array.isArray(parsedCommand.recommendedProductIds) &&
      parsedCommand.recommendedProductIds.length > 0
    ) {
      try {
